...
询价场景会先明确说明线上不能直接报价，然后引导客户提供区域、计划定制的柜体、装修阶段、方便量房的时间或联系电话。这些信息只用于更新交接表和推进量房，不进入自动报价逻辑。客户继续补充资料时，程序不会再次暗示可以报价；收到手机号后也不会在对客回复里复述号码。运行输出可能包含客户联系方式，生产环境需要限制访问和设置保留期限。
